window.onload = () => {
    ...
    fetchUsers = async () => {
        try{
            const result = await fetch(`${API_URL}/api/p/user/all`, {
                method:'GET',
                headers:{
                    'x-api-key': API_KEY
                }
    ...
    clickSendSMS= async (e) => {
        try{
            ...
            if(desc.trim() == ''){
                ...
                await sendLog(USER_ID, BRANCH_ID, 1, `<b>Kullanıcı üyeye sms gönderdi. <br /> İçerik: ${desc}</b>`);
    ...
    clickSendNotification = async (e) => {
        try{
            ...
            if(title.trim() == '' || desc.trim() == ''){
                ...
                await sendLog(USER_ID, BRANCH_ID, 1, `<b>Kullanıcı üyeye bildirim gönderdi. <br /> İçerik: ${desc}</b>`);
